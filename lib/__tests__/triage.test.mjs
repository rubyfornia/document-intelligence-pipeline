...
test("TOC page: a page-number rail is not a second column → stays deterministic", () => {
  const lines = [];
  for (let i = 0; i < 20; i++) {
    lines.push(mkLine("Chapter heading entry with a reasonably long title", 60, 90 + i * 30));
    lines.push({ text: String(8 + i), bbox: { x: 540, y: 90 + i * 30, w: 20, h: 14 }, fontName: "Helvetica", fontSize: 11, bold: false });
  }
  const t = triagePage(page({ lines }));
  assert.equal(t.route, "deterministic");
  assert.ok(!t.reason.includes("multi-column"), t.reason);
});
