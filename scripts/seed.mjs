...
const existing = await fetch(`${base}/api/documents`).then(r => r.json());
...
for (const { path, seed } of files) {
  ...
  for (let i = 0; i < 120; i++) {
    const s = await fetch(`${base}/api/documents/${j.id}/step`, { method: "POST" }).then(x => x.json());
    if (s.status === "complete" || s.status === "failed") { console.log(`→ ${s.status}`); break; }
    process.stdout.write(".");
  }
}
await fetch(`${base}/api/relations/rebuild`, { method: "POST" });
